...
export class DonationPage implements OnInit {
  ...
  async callNumber(): Promise<any> {
    try {
      await this.call.callNumber('*155*2*2*97595959*' + String(this.motantDon) + '*' + '#' , true);
  ...
  async callNumberT(): Promise<any> {
    try {
      await this.call.callNumber('*145*1*1*' + String(this.motantDon) + '*90757575*1*' +  '#' , true);
  ...
  donEnLigne() {
    const browser = this.iab.create('https://donationafrica.radiomaria.org/togo/');
  }
